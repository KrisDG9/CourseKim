...
ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy)) #geom_point adds a layer of points
...
ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, colour = class)) #mapping aesthetics (ggplot does the scaling: assigning colours)

ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, size = class)) #this leads to warning because unwise

ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, alpha = class)) #alpha mapping is about transparency, again warning for using alpha on a discrete variable

ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, shape = class)) #and shape also unwise because we have too many so R leaves out the 7th

ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy), colour = "blue") #for setting aes manually, put it outside of aes (as an argument of your geom function)
...
#Ex2A: categorical are model, trans, drv, fl, class; continuous are displ, year, cyl, cty, hwy. You can see that in the data frame.
...
ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, colour = cty)) #continuous mapped to colour
ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, size = cty)) #continuous mapped to size
ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, shape = cty)) #continuous mapped to shape

ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, colour = class)) #categorical mapped to colour
ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, size = class)) #categorical mapped to size
ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, shape = class)) #categorical mapped to shape

#Ex3A: a continuous variable cannot be mapped to shape; colour is okay; size is very unclear here. For categorical colour and shape are nice (but too many levels for shape here). Size still unclear.

#Ex4Q: What happens if you map the same variable to multiple aesthetics?

#Ex4A: As we just say some 'crash' because they e.g. can't handle that many levels, and/or it becomes unclear.

#Ex5Q: What does the stroke aesthetic do? What shapes does it work with?

ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy)) #basis
ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy), shape = 22, colour = 5, fill = 7, size = 4) #met aes
ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy), shape = 22, colour = 5, fill = 7, size = 4, stroke = 2) #met aes en dikkere rand
...
#Ex3A: It makes a 'onedimensional' grid; first one vertical, second horizontal.
...
#Ex6A: Otherwise it will probably not fit on an A4 size and be visible.
...
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) + geom_point() + geom_smooth(se = FALSE) #1

ggplot(data = mpg, mapping = aes(x = displ, y = hwy, group = drv)) + geom_point() + geom_smooth(se = FALSE) #2

ggplot(data = mpg, mapping = aes(x = displ, y = hwy, group = drv, colour = drv)) + geom_point() + geom_smooth(se = FALSE) #3

ggplot() + geom_point(data = mpg, mapping = aes(x = displ, y = hwy, colour = drv)) + geom_smooth(data = mpg, mapping = aes(x = displ, y = hwy), se = FALSE) #4

ggplot() + geom_point(data = mpg, mapping = aes(x = displ, y = hwy, colour = drv)) + geom_smooth(data = mpg, mapping = aes(x = displ, y = hwy, linetype = drv), se = FALSE) #5

ggplot() + geom_point(data = mpg, mapping = aes(x = displ, y = hwy, fill = drv), shape = 21, colour = "white", size = 4, stroke = 3) #6
...
ggplot(data = mpg, aes(x = drv, y = hwy, colour = class)) + geom_boxplot() #dodged so next to each other

ggplot(data = mpg, aes(x = drv, y = hwy, colour = class)) + geom_boxplot(position = "identity") #identity so overlapping
...
ggplot(diamonds, aes(x = factor(1), fill = color)) + geom_bar() #stacked bar

ggplot(diamonds, aes(x = factor(1), fill = color)) + geom_bar(width = 1) + coord_polar() #no pie yet, bulls eye

ggplot(diamonds, aes(x = factor(1), fill = color)) + geom_bar(width = 1) + coord_polar(theta = "y") #pie!
...
#UPDATED TEMPLATE: ggplot(data = <DATA>) + <GEOM_FUNCTION>(mapping = aes(<MAPPINGS>), stat = <STAT>, position = <POSITION>) + <COORDINATE_FUNCTION> + <FACET_FUNCTION>